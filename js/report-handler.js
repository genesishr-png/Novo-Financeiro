...
export class ReportHandler {
			...
			calculateIncomeByDateRange(startDate, endDate, contractsToRender) {
				...
				if (this.app.isUserAdmin) {
					// [NOVO] Integra��o de Receitas Avulsas
					...
					// [NOVO] Integra��o de Despesas do Escrit�rio
					...
					officeExpenses.forEach(exp => {
						if (exp.status === 'Pago' && exp.paymentDate) {
							...
							if (d >= startDate && d <= endDate) {
								...
								detailedPayments.push({ type: 'Despesa Escrit�rio', clientName: exp.description, date: d, value: -exp.value, advogado: '-' });
							}
						}
					});
				}
